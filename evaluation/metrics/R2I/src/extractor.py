            ...
            for k in sorted(score.keys()):
                ...
                if isinstance(val, dict):
                    ...
                    for subkey in sorted(val.keys()):
                        row.append(val[subkey])
                else:
                    row.append(val)
            row.append(" ")        
            rows.append(row)

        return pd.DataFrame(rows, columns=sorted(FEATURES_HDR) + [' '])            

def construct_default_dataframe(func_name):
    columns = [
        "_func_name", "asm", "assignment", "bitwise", "branch_case", "branch_if",
        "branch_ternary", "branch_total", "broken_dowhile", "cast", "comma",
        "cond_depth", "fix", "goto", "if_depth", "if_in_loop", "invalid_form",
        "invalid_funccall", "invalid_ir", "invalid_jump", "label", "loc", "local",
        "lol", "loop_depth", "loop_dowhile", "loop_for", "loop_total", "loop_while",
        "operator", "prop_branch_case", "prop_branch_if", "prop_branch_ternary",
        "prop_loop_dowhile", "prop_loop_for", "prop_loop_while", "ref_direct",
        "ref_structured", "strcmp_bare", "strcmp_not", "token", "undefined",
        "unimplemented", "unknown", "unknown_op", " "
    ]

    # 定义对应的一行数据（注意顺序与字段完全一致）
    data = [
        [
            func_name, math.inf, math.inf, math.inf, 0, math.inf, 0, math.inf, math.inf, 0, 0, math.inf, 1.0, math.inf, math.inf, math.inf, math.inf, math.inf, math.inf, math.inf, math.inf,
            math.inf, math.inf, math.inf, 0, math.inf, 0, 0, 1, 0, 0.0, 1.0, 0.0, math.inf, 0.0, 0.0, math.inf, 0, 0, 0,
            math.inf, 0, 0, 0, 0, " "
        ]
    ]
...
def extract(path, filename, debug, save, func_name):
    ...
    try:
        ...
        code.extract()
        return code.to_dataframe()     
    except:
        return construct_default_dataframe(func_name) 
...
def run_extraction(filename: str, path: str = "", debug: bool = False, save: bool = False, func_name = ""):
    # 保持行为一致
    # os.chdir(sys.path[0])

    # 如果同时打开 debug 和 save，我们模仿原脚本行为：打印用法并返回
    if debug and save:
        print("Usage: extractor.py [-h] [-d | -s] [-p path] filename")
        return

    return extract(path, filename, debug, save, func_name)        
