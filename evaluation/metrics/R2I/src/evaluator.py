...
class R2I():
    ...
    def collect_features(self, w, data: pd.DataFrame):
        ...
        weight = pd.read_csv(f'{os.path.dirname(__file__)}/weight.csv', header=0, delimiter=',')
...
def evaluate_targets(targets: list[str], weight: str, data: pd.DataFrame):
    r2i = R2I(targets)
    r2i.collect_features(weight, data)
    r2i.evaluate()
    if os.path.exists("aggregated.csv"):
