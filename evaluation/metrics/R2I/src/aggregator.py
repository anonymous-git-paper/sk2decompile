...
import pandas as pd
import argparse
from metrics.R2I.src.extractor import FEATURES_HDR
...
class Features():
    ...
    def save(self, experiment):
        target_name = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'eval', experiment, 'aggregated.csv')
...
def aggregate_features(experiment: str, targets: list[str], stripped: bool = False):
    extract_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'extract', experiment)
    os.chdir(extract_dir)
    f = Features(targets)
    f.collect(stripped)
    # f.save(experiment)
    f.scores.sort_values(by=['_binary', '_func_name'], inplace=True)
    return f.scores.copy()
